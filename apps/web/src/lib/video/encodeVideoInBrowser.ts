 ...
 *
 * ⚠️ הועלה ל-24 ואז הוחזר ל-10 יחד עם ה-livelock של ה-yield (ראה yieldToBrowser). הערך
 * הזה נבדק בפועל מקצה-לקצה; שינוי שלו דורש מדידה של קידוד מלא בדפדפן, לא הסקה.
 */
 ...
 *
 * ⚠️⚠️ **`setTimeout(0)` כאן הוא מכוון, וההידוק ל-4ms הוא תכונה ולא באג.** ניסיתי להחליף
 * אותו ב-`MessageChannel`/`scheduler.yield` כדי "לחסוך" את ההידוק — והתוצאה הייתה **livelock
 * בפרודקשן**: הקידוד נתקע על 0%. נמדד: yield כזה עולה ~3µs, כלומר `drainQueue` הסתובב
 * 16,000 פעם ב-47ms והציף את תור-המשימות. קריאות ה-`output` של המקודד הן **גם** משימות —
 * והן נרעבו, `encodeQueueSize` מעולם לא ירד, והלולאה לא התקדמה לעולם.
 *
 * ההשהיה של 4ms היא בדיוק מה שנותן לחוט-המקודד להספיק לעבוד. אל תחליפו אותה ב"מהיר יותר"
 * בלי למדוד קידוד מלא מקצה-לקצה בדפדפן אמיתי.
 */
