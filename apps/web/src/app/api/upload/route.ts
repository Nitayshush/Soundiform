 ...
 * input" בהעלאת תמונת-צילום מורכבת בפרודקשן): לא שגיאת-JS — ה-try/catch כאן היה תופס כל
 * חריגה אמיתית ומחזיר JSON תקין. גוף-תשובה ריק/חתוך פירושו של-Vercel **הרג את הפונקציה**
 * לפני שהיא סיימה — פונקציה שרצה יותר מדי זמן, ו-posterizeToSvg (rasterToShapeData.ts) מריץ
 * potrace **ארבע פעמים** (פעם לכל רמת-בהירות) על תמונה שהוקטנה ל-1024px; על צילום עתיר-
 * פרטים (לא ציור/לוגו נקי) זה יכול לחרוג בקלות מהתקרה. לא תיקון מושלם לתמונה קיצונית-במיוחד
