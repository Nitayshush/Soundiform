...
## 2026-08-22 — איכות המוזיקה, רגאיי, סרגל-תווים ויראלי, הורדות-וידאו, תיקון-ציור

Nitay פתח בביקורת חדה וישירה: "המוצר שלנו יעבוד רק אם יצאו קטעי מוזיקה מעולים" — הסגנונות
לא נשמעים שונים, לוח הציור עדיין לא שומר על הצורה המדויקת, אין רגאיי, סרגל התווים משעמם
ויזואלית, וההורדה לא מובילה לקובץ-וידאו-ליוטיוב. ביקש תכנון-על לפני ביצוע, ואז אישר לרוץ על
הסשן כולו ברצף מלא עד סיום או גמר טוקנים, בלי לעצור לשאלות.

**מחקר לפני תכנון** (סוכן Explore ייעודי) חשף שהבעיה אמיתית ומדויקת, לא תחושה: `harmonyEngine.ts`
הריץ את _אותה_ התקדמות הרמונית (I–vi–IV–V) על כל חמשת הסגנונות, תמיד טריאדות נקיות למרות
ש-2 מהם הצהירו `harmonicTendency:"extended"` (שדה מת, אף אחד לא קרא אותו); כל pack חיבר
`rhythmPatterns.bass/lead/pad/skank` עשירים ומובחנים-לסגנון ב-JSON, אבל `genreAdapter.ts`
קרא רק `.drums` — הבס והמלודיה תמיד ניגנו במרווח-אחיד גנרי; טראנס והאוס שיתפו תבנית-תופים
זהה בייט-לבייט תחת שמות שונים; `sidechain.ts` (חתימת-הטראנס) היה stub ריק מאז Sprint 4;
אין שום פילטר בכל `packages/audio`; רגאיי חסום כי דורש samples ו-`SamplerProvider` שלד ריק.

בוצע כ-8 פריטים נפרדים, כל אחד commit+sweep+בדיקה-חיה נפרדים:

**1. תיקון-ציור אמיתי:** `useShapeCapture.ts`'s paper.js `Path.simplify()` הוא least-squares
Bézier curve-fit — לא סינון-רעש, בונה-מחדש את המסלול כעקומה חלקה עם מעט נקודות+ידיות; הקוד
שמר רק את הנקודות (בלי הידיות) ו-`DrawingCanvas.tsx` מצייר קווים ישרים ביניהן, אז גם fit
"נכון" נראה "מיושר". תיקון הטולרנס בסבב הקודם עזר רק לפוליגונים חדי-פינות. הוחלף ב-**RDP**
(Ramer–Douglas–Peucker) עצמאי — כל נקודה ששורדת יושבת בפועל _על_ המסלול המקורי, לא בנייה-
מחדש. הסיר את התלות ב-paper.js לגמרי. **נבדק חי:** קשת חלקה אמיתית (46 נקודות) נשארת חלקה,
לא מתפרקת לפאונים; זיגזג חד-פינות עדיין חד; דטרמיניזם (אותו קלט→אותו shapeHash) מאושר.

**2. הרמוניה אמיתית לפי-סגנון:** `chordProgression` (מערך דרגות-סולם) נוסף ל-GenrePack,
מחליף את הקבוע ה-hardcoded — טראנס/האוס מקבלים modal vamp (i-VII-VI-VII / i-IV-VI-V), צ'יל
מקבל ii-V-I-vi ג'אזי, קולנועי i-VI-III-VII, רגאיי I-V-vi-IV. `buildChord` חדש (לצד `buildTriad`
הקיים, לא נגע בו) מוסיף 7th כש-`extendedChords` (מ-`harmonicTendency`) דלוק.

**3. צריכה אמיתית של תבניות-הריתמיקה (התיקון הכי-משפיע):** `genreAdapter.ts` מעביר עכשיו
`rhythmPatterns` מלא (לא רק drums) ל-`CompositionConfig`. `buildBassTrack` מנגן על פגיעות
התבנית בפועל (עדיין pitch-של-בר-שלם, רק ריתמיקה שונה); `buildLeadTrack` שומר על _כמות_ התווים
מהצורה (§4.5, נבדק) אבל "דוגם" אותם לתוך מיקומי-הסינקופציה של הסגנון. `buildSkankTrack` חדש
לרגאיי (צ'ופים על 2+4, לא pad מתמשך — לרגאיי אין pad בכלל עכשיו).

**4. Arrangement אמיתי:** intro/build/outro אמיתיים (לא רק `["loop"]` שהיה תמיד) — ה-loop
עצמו נשאר באורך-מהצורה, אבל build (טראנס/האוס/קולנועי) מוסיף עלייה-באנרגיה + מילוי-תופים,
intro/outro מוסיפים "נשימה" שקטה על פני/סקאנק. **באג אמיתי נתפס בדרך:** מילוי-תופים ב-build
ניסה להכפיל צפיפות דרך חצי-צעד (32nd) — הפר את כלל "הכל מקוונטז לגריד" (§4.3) מול ה-
gridSubdivision המוצהר; תוקן למילוי-כל-הצעדים על אותו גריד.

**5. עיצוב-סאונד אמיתי:** `sidechain.ts` (stub) מומש בפועל — gain node משותף שנדחק בכל
פגיעת-קיק ומתאושש (מתוזמן ישירות מתזמון-התופים הסימבולי, דטרמיניסטי לגמרי). `SynthProvider`
מקבל פילטר אופציונלי לפי-קול (lowpass/highpass, cutoff/resonance) ורוחב-unison מוגדר לפי-
סגנון (היה קבוע גלובלי אחיד). כל סגנון קיבל טיונינג אמיתי: טראנס lead = supersaw רחב (7
קולות) + פילטר בהיר; רגאיי skank = highpass (בלי תוכן-בס, כמו גיטרה אמיתית!); קולנועי pad
= lowpass חמים.

**6. רגאיי הופשר:** עם התשתית של 2-5 מוכנה, `requiresSamples` הפך ל-`false` — קירוב סינתטי,
לא ממתין ל-samples אמיתיים. עדכן PROJECT.md + בדיקות שהיו מניחות "4 סגנונות פעילים".

**7. סרגל-תווים ויראלי:** זוהר (glow) על תווים+קו-סורק דרך `BlurFilter` המובנה ב-pixi.js
core (בלי תלות חדשה), פרצי-אור בכל חציית-תו, פעימת-רקע לפי אנרגיה נוכחית — הכל על
`app.ticker` (אנימציה מתמשכת). פורט אותו שפה חזותית ל-`frameRenderer.ts` (canvas 2D's
shadowBlur/shadowColor במקום Pixi filters) כדי שהוידאו-המורד יראה אותו דבר.

**8. "הדלת הקדמית" החסרה:** גילוי אמיתי במהלך התכנון — **שום דבר ב-UI מעולם לא קרא ל-
`POST /api/render`** (רק בדיקות/scripts ישירים לאורך כל הפרויקט)! נבנה `GET /api/render/
[jobId]/status` (BullMQ job.getState, לא היה קיים), `type=video` נוסף להורדה, ו-`useDownload.ts`
חדש (מורכב על `useSaveProject.ts` הקיים — לא כפילות) שמפעיל render→share→download אמיתי,
עם וידאו כברירת-מחדל (16:9, ליוטיוב) ואודיו כמשני. אנונימי מקבל redirect-and-resume (autoSave=1
+autoDownload=1 מקביל). Studio מקבל כפתור "Download" ראשון-אי-פעם.

**שני באגים אמיתיים נתפסו ע"י בדיקה חיה ייעודית** (רינדור-כל-5-הסגנונות + ניתוח waveform
בפועל, לא רק "זה לא קרס"): (א) `normalizeToTargetLufs` דחף gain לפי RMS/LUFS בלי לבדוק
פיקים — הפילטרים הרזוננטיים/unison הרחב של פריט 5 העלו קרסט-פקטור והביאו לקליפינג אמיתי
(0.02%-0.29% מהסמפלים); תוקן עם peak-limiting אחרי חישוב ה-gain. (ב) `computeDurationSeconds`
לא הקצה זמן ל-reverb tail — outro של צ'יל/קולנועי (עם ריוורב 4-5 שניות) נחתך באמצע-דעיכה;
תוקן עם תוספת-זנב לפי `mixCharacter.reverbDecaySeconds` בפועל.

**נבדק חי לאורך כל הסבב:** ארבע בדיקות Playwright/ffprobe/waveform-analysis נפרדות (תיקון-
ציור, רינדור-5-סגנונות+ניתוח-דאטה-אמיתי, ויזואל-הסרגל, זרימת-Download-קצה-לקצה) — typecheck/
lint/format/test מלאים נשארו נקיים אחרי כל commit.

**עדכון: בדיקת ה-Download-קצה-לקצה תפסה 2 באגים אמיתיים בפריט 8**, שני אלה עצמם תוקנו
מיד: (א) `useSaveProject.ts`'s תיעוד תיאר יכולת `extraNextParams` ל-`requestSave` — אבל
המימוש בפועל **לא** קיבל את הפרמטר בכלל (נכתבה הכוונה, נשכח היישום)! המשמעות: `autoDownload=1`
מעולם לא הצטרף ל-redirect, אז הזרימה "לחץ Download כאנונימי → תתחבר → הכל ממשיך לבד" לא
עבדה בכלל. תוקן: `requestSave` מקבל עכשיו `extraNextParams` באמת, ו-`useDownload.ts` קורא
`requestSave('autoDownload=1')`. (ב) `window.location.href` להורדה לא הוריד שום קובץ בפועל —
ה-URL החתום מ-R2 חסר `Content-Disposition: attachment`, אז הדפדפן פשוט _ניווט_ ל-URL וניגן
את הוידאו inline, עוזב את האתר במקום להוריד ולהישאר. תוקן ברמת ה-abstraction (לא רק המקרה
הזה): `StorageProvider.DownloadUrlOptions` מקבל `responseContentDisposition` חדש, מועבר
דרך S3's `ResponseContentDisposition` ב-`R2Provider`, ומוגדר בפועל ב-`api/renders/[id]/
download/route.ts` לכל סוג-הורדה (audio/video/midi/stem) — מתקן גם את "Download audio"
הקיים בדף השיתוף, לא רק את הכפתור החדש. גם `MAX_POLL_ATTEMPTS` הועלה מ-60 (2 דק') ל-150
(5 דק') — רינדור וידאו אמיתי (720p, ~18 שניות מוזיקה) נמדד ב-~170-176 שניות, קרוב-מדי לגבול
הישן.

## 2026-08-22/23 — גלריה + חברתי: הציור בוידאו, ייחוס-יוצר, לייקים/תגובות/My Gallery/עוקבים

**הקשר:** בקשה מפורשת של ניתי (5 סעיפים) לתכנן-ואז-לבצע שדרוג לגלריה ולשמירת-וידאו: (1) שמירת
render אחת = סגנון אחד בלבד; (2) הוידאו שיורד/נשמר יכיל בפועל את הציור/תמונה שהמשתמש עשה
(היום לא); (3) גלריה כללית ממוינת-צפיות עם שם-יוצר+קישור-לפרופיל+ליצירותיו; (4) טאב "My
Gallery" ליד Account; (5) לייק/מעקב/שיתוף/תגובות על כל וידאו + מנגנון-מעקב-פעיל (רשימת-מי-
אני-עוקב עם קישורים). תוכנן קודם עם ExitPlanMode (כולל רציונל מפורש — "למה בכלל יש גלריה"),
ואז בוצע כ-7 items, כל אחד commit נפרד עם typecheck/lint/format/test מלא ואימות-חי.

**מיפוי סעיף→עבודה בפועל:** (1) התברר _כבר נכון_ מבנית (renders.genreId=סגנון-יחיד, POST
/api/render מקבל genreId יחיד, החלפת-סגנון ב-Studio היא preview דפדפני בלבד, בלי round-trip
לשרת) — אין שינוי pipeline, רק וידוא שהתצוגה (item 3) לא מטשטשת את זה. (2)-(5) = items 1,3,4,5,6,7 למטה.

**Item 1 — "שרטוט מסונכרן" של הצורה בתוך הוידאו (הפריט המרכזי):** `frameRenderer.ts`'s
header עצמו תיעד ב-2026-08-21 החלטה מפורשת "וידאו לא צריך shape, סרגל-התווים מספיק" — הסבב
הזה הפך אותה, כי מנגנון-הצמיחה (§9) תלוי בזה שהקליפ _שבאמת מופץ_ (וואטסאפ/אינסטגרם/יוטיוב,
לא האתר) יראה את הציור, לא רק תוצאה מופשטת. נשאלה שאלת-עיצוב מפורשת (AskUserQuestion, 3
אפשרויות: רקע-זוהר-קבוע / כרטיס-פתיחה / שרטוט-מסונכרן-מתקדם) — ניתי בחר באפשרות השלישית,
היקרה-ביותר לביצוע אך הכי-נאמנה למטאפורה של המוצר ("הציור הופך למוזיקה על ציר-זמן"). מומש
כמודול גיאומטריה טהור **משותף** `packages/shared/src/shape/shapeReveal.ts`
(`computeShapeLayout`/`revealedSegments` — הקרנה לריבוע ממורכז + טבלת-אורך-מצטבר לכל
מקטע), נצרך גם ב-`apps/worker/src/video/frameRenderer.ts` (הוידאו המיוצא) וגם ב-
`ScoreStaff.tsx` (הפריוויו החי בדפדפן) — אותה מתמטיקה בדיוק, "פריוויו ≈ פלט סופי" ויזואלי
לא רק שמיעתי. `shapeData` הוזרם מחדש דרך כל שרשרת ה-render job (`RenderJobData` ב-
`packages/audio`, עד `renderVideo.ts`/`videoEncoder.ts`) — נדרש reversal מלא של ה-2026-08-21
comment. תוצר-לוואי: `poster.jpg` (פריים ב-progress=0.5, `renders.posterKey` חדש) — thumbnail
לכרטיסי-גלריה (item 3), בלי לצטרך ניגון-וידאו-חי בגריד-גלילה. **אומת חי**: רינדור אמיתי
דרך BullMQ+worker+R2 אמיתיים (כוכב 10-נקודות), פריימים חולצו ב-ffmpeg ב-~0%/50%/99% — 0%
ריק, 50% חצי-משורטט (קו פתוח), 99% כוכב סגור מלא. תקין, אין באגים.

**Items 2/3 — ייחוס-יוצר + embedding אמיתי:** `GalleryCard` משותף חדש (poster thumbnail,
genre badge, views+likes, שם-יוצר+אווטאר עם קישור ל-`/u/[username]`, כפתור-מעקב inline)
מחליף שלושה כרטיסים כמעט-זהים (גלריה ציבורית, גריד-יצירות בפרופיל) — היו כפולים לגמרי.
דף השיתוף (`/s/[shareId]`) מקבל `<video>` אמיתי (עם poster) כשקיים, fallback ל-`SharePlayer`
הישן (ניגון-חי-בדפדפן, בלי הצורה) ל-renders ישנים בלי וידאו. `api/renders/[id]/download`
מקבל `type=poster` ו-`inline=1` (מדלג Content-Disposition:attachment) כדי שאותו signed-URL
ישרת גם `<video src>`/`<img src>`, לא רק הורדה בפועל. `DownloadLinks` מקבל "Download video"
לראשונה (היה קיים רק בזרימת-ה-Download של Studio עצמו, לא נגיש מדף שיתוף/פרופיל).

**Items 4/5 — לייקים (schema-only→מלא) + תגובות (חדש-לגמרי):** `likes` הייתה טבלה בלי שום
API/UI מאז Sprint 8 — עכשיו `POST`/`DELETE /api/likes` (זהה בדיוק לתבנית `api/follows`) +
`LikeButton` (מונה+heart) בדף השיתוף, ומונה-לייקים read-only בכרטיסי-גלריה. `comments` טבלה
חדשה-לגמרי (per-render, לא per-project — renders שונים של אותו project יכולים לקבל תגובות
נפרדות) + `GET`/`POST /api/comments`, `DELETE /api/comments/[id]` (תגובה-עצמית או אדמין,
לא RLS policy — כמו נתיבי-admin אחרים). מודרציית-V1 מינימלית _במכוון_: הגבלת-אורך (1000
תווים) + מחיקה-עצמית/אדמין, בלי תור-מודרציה מקדים ובלי rate-limiter — ייבנה רק אם וכאשר
יתגלה ניצול-לרעה בפועל.

**Items 6/7 — "My Gallery" + מנגנון-מעקב-פעיל:** `/account/gallery` חדש (טאב בהדר ליד
Account) — כל render ששמר המשתמש (כל visibility, לא רק public — `useDownload.ts` כבר יוצר
share אוטומטית עם `visibility:'public'` בכל הורדה, אז זו כבר כמעט הרשימה המלאה). `/account/
following` + `/account/followers` חדשים (סימטריים, join הפוך על `follows`) — לפני זה היה
קיים רק _מספר_ עוקבים בפרופיל הציבורי, בלי רשימה בפועל לגלוש בה. אגב-תיקון: `/account`
עבר מ-`users.plan` ישיר ל-`resolveEffectivePlan()` (עקבי עם מענק-הגישה-הזמני שנבנה קודם
באותו יום — בלי זה, plan היה יכול להיראות מיושן לרגע אחרי שמענק פג).

## 2026-09-01 — סבב ג': ארנג'מנט כבחירת-משתמש, וגיטרות CC0

**הבקשה שינתה את התכנון.** סבב ג' תוכנן כשינוי-מנוע שקוף (כלים נכנסים ויוצאים מעצמם).
ניתי ביקש במקום זאת **כפתור בפאנל הסאונד** — "עוד רובד של עריכה או בחירה". זה עדיף משתי
סיבות: המשתמש מקבל שליטה, וברירת המחדל ("מלא") היא בדיוק ההתנהגות הקיימת, כך שאף יצירה
קיימת לא משתנה אלא אם בוחרים אחרת. שורה שלישית ב-`BeatAndKeyRows` לצד מקצב וסולם.

**המימוש הוא פוסט-פרוססור טהור** (`packages/core/src/arrangement/arrangementPattern.ts`,
התיקייה שהייתה ריקה מ-Sprint 3 עם TODO ב-`index.ts`), שרץ **בסוף** `composeMusicalScore`
על הטראקים המוכנים ולא בתוך בונה-טראק כלשהו. שתי סיבות: אף בונה לא צריך להכיר את המושג,
ולכן אין סיכון שאחד יחיל ואחר לא; ופונקציה שרק **מסירה תווים ומכפילה וולוסיטי** אינה
_מסוגלת_ לשנות גובה. ⚠️ זה נועד למנוע חזרה של הבאג מ-24.8 (היסט-אוקטבה ב-intro/outro
בשם "דינמיקת-ארנג'מנט") ששבר נאמנות-לצורה ובוטל אחרי בדיקה חיה. יש על זה בדיקה.

**ה-`loop` מחוץ לתחום — מבנית.** `ArrangementStageName` הוא `'intro' | 'build' | 'outro'`
בלבד, כך שאי אפשר בכלל לכתוב תבנית שמרוקנת את התוכן שנגזר מהציור. זו גם הערובה שיצירה
לא יכולה לצאת שקטה.

**הדילול לפי חוזק מטרי ולא אקראית** — דאונביט וחצאים נשארים, מה שביניהם יורד; זה מה שנגן
עושה כשהוא "מנגן פחות", והוא דטרמיניסטי לחלוטין בלי RNG.

**⚠️ שני באגים קיימים שנמדדו בדרך (לא נגרמו מהסבב, לא תוקנו בו):**

1. **תוכן שמתרכז בקצוות.** בצורות חלקות ליד/בס/תופים כמעט נעלמים מאמצע היצירה: עיגול
   בטראנס נותן אותם בברים 0,1,9 בלבד מתוך 10 — 7 ברים של פאד בלבד. נמדד כיסוי של 30%–60%
   בעיגול וריבוע בטראנס/האוס/צ'יל/סינמטי, מול 100% ברגאיי (שאינו במסלול הרסטר). הסיבה
   ככל הנראה מבנית: האונסטים נגזרים מאירועים גיאומטריים, ולעיגול יש מעט שינויי-גובה
   באמצע. בגלל זה נוספה רשת-ביטחון: תבנית שהייתה משתיקה תפקיד **לגמרי** מוותרת ומשאירה
   אותו כמו שהיה.
2. **תווים אחרי סוף היצירה.** ב-`house/diagonal`, `house/square`, `chill/diagonal` ו-
   `cinematic/square` יש תווים ב-`startTick === durationBars * TICKS_PER_BAR` — בדיוק
   על הגבול, מחוץ לטווח המוצהר. הסכימה לא תופסת את זה.

**גיטרות — הפער נסגר.** שתי ערכות CC0 מ-FreePats, רישיון נקרא מתוך הארכיון. `electric-guitar`
(פיקאפ גשר) כאופציית `skank` ברגאיי לצד הסינת' הקיים שנשאר, ו-`nylon-guitar` כאופציית
`lead` בצ'יל ובסינמטי. ⚠️ **הוספת אופציה משנה את ברירת המחדל לפי-צורה** (`seededIndex`
על מספר האופציות), כך שיצירות קיימות בשלושת הסגנונות האלה עשויות לקבל צליל אחר.

⚠️ **7z**: לא היה צריך להוריד מחלץ — ל-Windows יש `bsdtar` ב-`System32	ar.exe` שקורא 7z.
`7zr.exe` הרשמי דווקא **נכשל** על שני הארכיונים (BZip2 ו-Deflate, שהגרסה המצומצמת לא תומכת).

⚠️ **הנרמול חייב להימדד על הפלט.** ראה `docs/SAMPLES.md` — קידוד mp3 חורג מהשיא של המקור,
והגיטרה החשמלית הייתה נכנסת 1.4dB חזקה משאר הספרייה.

## 2026-09-01 (המשך) — רגאיי עובר לציור, והסורק מסונכרן למוזיקה

**החלטת-מוצר מפורשת של ניתי, ששינתה את מה שנחשב "באג".** הצגתי כבאג את זה שבצורות חלקות
ליד/בס/תופים נעלמים מאמצע היצירה (עיגול בטראנס: ברים 0,1,9 מתוך 10). התשובה:

> "אם יש שקט באמצע העיגול, זה הסאונד שלו, פשוט ככה. המשתמש יפתח את הציור וככה יתמודד עם
> השקט. המטרה לייצר חותמת סאונד לכל ציור וצורה."

כלומר זו **לא** תקלה אלא הביטוי של עקרון-הליבה. לכן לא תוקן — ובמקום זה **רגאיי הועבר
לאותו מנגנון** (`absoluteNoteBoard: true`), הסגנון האחרון שנשאר על המסלול הישן של מתאר-גובה
ממוצע ופרוגרסיה קבועה. נמדד אחרי המעבר: טווח-גבהים ממוצע בליד 7.6 (טראנס: 9.6), וחמישה
ציורים שונים → חמש יצירות שונות.

⚠️ **המעבר לבדו היה הורס את זהות הסגנון.** במסלול הרסטר התופים נגזרים מהציור, כלומר הוואן-דרופ
היה נעלם. לכן נוספו לרגאיי גם `beatPatterns` (`one-drop`, `steppers`, `rockers`) — בדיוק
הפתרון של סבב א' לטראנס/האוס: הציור קובע אילו תאים, הסגנון קובע איך הם יושבים בזמן. נוסף גם
`soundOptions.drums` עם הערכה האקוסטית, כי `autoSelectKitForBeat` בוחר ערכה רק מתוך
`soundOptions`, ובלעדיה בחירת ביט הייתה משמיעה סינת' שמתעלם מ-`drumPiece` (ה"ביפ" המתועד).
`beatAccents` של רגאיי הוגדרו `[0.72, 0.94, 1, 0.94]` — פעמה 3 החזקה ביותר, פעמה 1 החלשה,
שזה הוואן-דרופ עצמו.

**הסורק והמוזיקה — הבאג האמיתי היה בסנכרון, לא בציון.** דווח: "המוזיקה מושתקת לפני שהסורק
מגיע לסוף הלוח". נמדד שהציון מלא עד 100% מהציר בכל הסגנונות, כלומר המוזיקה לא נגמרת מוקדם.
מה שקרה: `progress = currentSeconds / durationSeconds`, ו-`computeDurationSeconds` הוא
**נומינלי + זנב-ריוורב** — והזנב הוא שקט מבחינת תווים. הסורק פשוט המשיך לנוע על פני הדעיכה
(בסינמטי: עד 27% מהמסע ביצירה קצרה).

תוקן ב-`computeMusicalDurationSeconds`/`scannerProgress` חדשים ב-`@soundiform/audio`, ו-
**שלושת** מקומות-הציור עברו אליהם: הסטודיו החי, הווידאו בדפדפן והווידאו בוורקר. ⚠️ שלושתם
חישבו התקדמות בנפרד; מקור-אמת אחד הוא מה שמונע "פריוויו ≠ פלט". בנוסף `MAX_RELEASE_TAIL_SECONDS`
הוקטן 8→3 לפי הבקשה ("שהסיומת תהיה ב-2-3 השניות האחרונות") — זה חותך רק את הריפוד, לא את
הריוורב עצמו, ובנקודה הזו הדעיכה כבר מתחת ל--30dB.

**בדיקה חדשה שנועלת את העיקרון**: `shapeFingerprint.test.ts` — כל סגנון חייב להיות על הלוח
האבסולוטי, ארבעה ציורים שונים חייבים לתת ארבע יצירות שונות, ואף צירוף סגנון × מקצב × צורה
לא מפיל את הבנייה. ⚠️ הבדיקה בודקת **שונוּת** ולא צפיפות מינימלית, בדיוק כי שקט הוא תוצאה
לגיטימית של הציור.

## 2026-09-01 (סוף היום) — הארנג'מנט הוסר, והביט של הסגנון הפך לברירת המחדל

**הארנג'מנט הוסר לבקשת הפאונדר, אחרי בדיקה חיה.** הוא נבנה באותו יום, ולכן ההסרה כוללת את
כל מה שנוסף: `packages/core/src/arrangement/`, השדה ב-`CompositionConfig`, ה-`arrangementId`
ב-`CreationSettings`/`ProjectCreationSettings`/`CompositionOverrides`, `arrangementPatterns`
בסכימת הסגנונות ובחמש החבילות, השורה בפאנל, והבדיקות שלו.

⚠️ **למה זה נכשל, וזה שווה תיעוד כדי לא לחזור על זה:** הפיצ'ר יכול היה **רק להסיר תווים**
מהכלים שכבר נקבעו — הוא רץ כפוסט-פרוססור בסוף `composeMusicalScore`, אחרי שהאינסטרומנטציה
כבר סגורה. בגרסה הראשונה הוא הוריד צפיפות עד 0 והפאונדר דיווח "סתם הופך את המנגינה לאיטית
ומשעממת". איזנתי אותו כך שאף תפקיד לא יורד מ-0.5 — ואז נמדד שהוא משנה 16-21% מכמות התווים
בלבד, כלומר כמעט לא נשמע. שתי הגרסאות היו גרועות מאותה סיבה מבנית.

**המודל שהפאונדר תיאר הוא "להקה"** — כלים שנכנסים ויוצאים. זה לא מה שהפיצ'ר עשה, וגם לא מה
שהוא _יכול_ היה לעשות במיקום שבו הוא רץ. ארנג'מנט אמיתי חייב להחליף **כלים** בין הסקשנים,
כלומר לרוץ _לפני_ קביעת האינסטרומנטציה ולא אחריה. מה שכן מייצר תחושת להקה היום הוא **בחירה
מרובה לכל תפקיד** (ליד נושא אורגן + סקסופון + פסנתר בו-זמנית, אומת) — והיא כלי אחד לתפקיד
בברירת מחדל. זו נקודת ההתחלה לכל ניסיון עתידי.

**שינוי גדול יותר שנשאר: הביט של הסגנון הוא ברירת המחדל.** בורר הביט נפתח על "מהציור",
ולכן `beatPattern` היה undefined והתופים נגזרו כולם מהרסטר. נמדד על ציור אמיתי: **טראנס
הפיק קיק אחד ביצירה שלמה**, האוס אחד, רגאיי שניים — פיזור הקשה בלי דופק. הביטים לפי-סגנון
נבנו בסבב א' בדיוק בתור החריגה המכוונת מ"הכל מהציור" ("קצב מסגרת"), והם היו כבויים אצל כל
מי שלא פתח את הפאנל. אחרי: טראנס 16 קיקים, האוס 16 קיקים ו-48 היי-האט, רגאיי 9 ו-51.
"מהציור" נשאר, אבל כ**בחירה מפורשת** — ולכן הסמל `DRAWING_BEAT_ID` עבר מה-store (client-only)
ל-`genreAdapter`, שגם מסלולי הרינדור בשרת קוראים.

**רגאיי:** הסקאנק ישב על פעמות 2 ו-4 (שתי נגיחות בבר) במקום על ה-& של כל פעמה — זה מה
שנשמע "לא רגאיי". עבר ל-2/6/10/14, והבס קיבל תבנית מסונקפת בת 6 מכות במקום 2. הסקאנק גם
נבנה עכשיו על **כל** הברים ולא רק על ה-loop (`buildSkankTrack` קיבל `fullProgressionDegrees`
בלי הזזה) — קודם הוא כיסה בר אחד משלושה, וה-intro נפתח בלי האלמנט שמגדיר את הסגנון.
סדר הביטים שונה כך ש-**Rockers** הוא ברירת המחדל; One Drop דליל בקיק במכוון ונשמע איטי.

⚠️ **שתי בדיקות קיימות נכשלו מהשינויים האלה, ושתיהן צדקו.** התקרה של 8 מכות-תוף לבר נכתבה
כשהתופים היו קולות סינת' בסינתזה בזמן-אמת, והיא תפסה עכשיו גם את התבנית המכוונת של הסגנון;
היא נמדדת מעכשיו **במפורש על המסלול שנגזר מהציור** (`DRAWING_BEAT_ID`), זה שאין לו גבול
עליון מובנה, לצד בדיקה רחבה חדשה על כל הסגנונות. הבדיקה השנייה קיבעה ש"בלי מקצב לא נטענת
ערכה" — כלל שהתהפך מאותה סיבה.
