...
ה-URL החתום מ-R2 חסר `Content-Disposition: attachment`, אז הדפדפן פשוט *ניווט* ל-URL וניגן
...
**מיפוי סעיף→עבודה בפועל:** (1) התברר *כבר נכון* מבנית (renders.genreId=סגנון-יחיד, POST
...
הזה הפך אותה, כי מנגנון-הצמיחה (§9) תלוי בזה שהקליפ *שבאמת מופץ* (וואטסאפ/אינסטגרם/יוטיוב,
...
לא RLS policy — כמו נתיבי-admin אחרים). מודרציית-V1 מינימלית *במכוון*: הגבלת-אורך (1000
...
קיים רק *מספר* עוקבים בפרופיל הציבורי, בלי רשימה בפועל לגלוש בה. אגב-תיקון: `/account`
