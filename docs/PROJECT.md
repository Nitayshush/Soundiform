...
│   │   │       ├── house.json
│   │   │       ├── chill.json
│   │   │       ├── reggae.json       # פעיל ב-V1 (קירוב סינתטי — ראה §9)
│   │   │       └── cinematic.json
│   │   └── package.json
│   │
│   ├── storage/                      # ⭐ הפשטת אחסון
...
⭐ **עדכון 2026-08-22**: רגאיי פעיל ב-V1 דרך קירוב סינתטי (`requiresSamples: false`) — סקאנק
מנוגן כ-square מסונן highpass (בלי תוכן-בס, כמו גיטרת סקאנק אמיתית), בס נקי בלי unison,
one-drop אמיתי דרך rhythmPatterns. אורגן/גיטרה דגומים-אמיתיים עדיין לא קיימים (SamplerProvider
נשאר שלד ריק) — שדרוג עתידי אפשרי, לא נדרש כדי שהסגנון יישמע אמיתי ופעיל היום.
