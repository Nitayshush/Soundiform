 ...
 *
 * ⚠️ **2026-08-31 — הערך היה 1ms, וזה היה קטן מדי.** ההערה למעלה כבר תיארה את הבעיה
 * הנכונה (הצמדה לבלוק-עיבוד של ~4ms) אבל הסף לא כיסה אותה: שני תווים במרחק 1–4ms עברו את
 * ההגנה, נצמדו לאותו בלוק, ו-`Source.start` זרק "Start time must be strictly greater than
 * previous start time" — כשל שמפיל את **כל** הרינדור. נמדד אחרי סבב מנגנון-הקצב: 270 זוגות
 * כאלה ב-80 ציורים אקראיים בטראנס, כי הצמדת-המדיניות מרכזת מכות ואז ההומניזציה מפזרת אותן
 * במילישניות בודדות.
 *
 * 12ms נבחר כדי לכסות בלוק-עיבוד בכל קצב-דגימה שאנחנו מרנדרים בו, בשוליים. הוא עדיין רחוק
 * מאוד מהתו הקצר ביותר שמנגנים בפועל (שלושים-ושתיים ב-138BPM ≈ 54ms), ולכן לא מדלג על אף
 * תו מוזיקלי אמיתי — רק על תווים ששום קול מונופוני לא היה יכול להשמיע בנפרד ממילא.
 */
const MONOPHONIC_MIN_SEPARATION_SECONDS = 0.012;
