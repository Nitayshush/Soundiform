  ...
  beatPatterns: z
    .array(
      z.object({
        ...
        /**
         * ⭐ 2026-09-01: הצעדים שבהם **הבס** רשאי לנחות בגרוב הזה (0-15 בתוך הבר).
         *
         * ⚠️ זה לא קו-בס קבוע — הציור עדיין בוחר אילו מהצעדים האלה באמת מתנגנים ובאיזה
         * גובה. הגרוב קובע רק **איפה מותר**, בדיוק לפי "הציור קובע אילו תאים, הסגנון קובע
         * איך הם יושבים בזמן". בלי זה בורר-הביט שינה רק את התופים, וכשני-שלישים מהיצירה
         * (בס, סקאנק, ליד) יצא זהה בכל הביטים — נמדד, וזה מה שנשמע כ"אין הבדל".
         *
         * ⚠️ הכוונה היא שהבס ידגיש מקומות **אחרים** מהתופים; זה מה שיוצר את השילוב
         * המהפנט שמאפיין רגאיי.
         */
        bassSteps: z.array(z.number().int().min(0).max(15)).min(1).optional(),
        /**
         * ⭐ 2026-09-02: פיגורת ה**סקאנק** של הגרוב — וולוסיטי לכל צעד, כמו `pieces`.
         *
         * ⚠️ **למה זה הכרחי.** נמדדה חלוקת האנרגיה במיקס: הסקאנק הוא **50%-59%** ממנה,
         * והוא היה זהה בכל הביטים. כלומר יותר מחצי ממה שהמשתמש שומע לא השתנה כשהוא החליף
         * ביט — ולכן שום שינוי בתופים (17%-31%) לא הורגש. אלמנט דומיננטי שאינו משתנה מבטל
         * את כל הבורר.
         */
        skank: z.array(z.number().min(0).max(1)).length(16).optional(),
      }),
    )
    .optional(),
});
